...
movie_data = api.model(
    'Movie Data',
    {
      "poster": fields.String(description="Jenre type", required=True),
      "Movie title": fields.String(description="Movie name", required=True),
      "Jenre": fields.String(description="Jenre type", required=True),
      "director": fields.String(description="Jenre type", required=True),
      "actor": fields.String(description="Jenre type", required=True),
...
class movies_jenre_model(Resource):
  def get(self, jenre, jenre_id):
    ...
    if not jenre_id in movie_info[jenre].keys():
      abort(404, description=f"jenre ID {jenre}/{jenre_id} doesn't exists")
  ...
  def post(self, jenre, jenre_id):
    ...
    if jenre_id in movie_info[jenre].keys():
      abort(409, description=f"jenre ID {jenre}/{jenre_id} already exists")
